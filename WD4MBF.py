...
def banner():
	clear()
	wel = '# FACEBOOK INDONESIA 2 0 2 0'
	wel2 = mark(wel, style='cyan')
	sol().print(wel2)
	au='AUTHOR    :  WIDI SANTOSO\nGITHUB    :  https://github.com/WIDI\nWHATSAPP  :  089512664026'
	pengembang1=nel(au,style="bold green")
...
def result():
	...
	sol().print(mark(cek, style='bold green'))
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s WIDI:%s %s/%s [OK:%s][CP:%s]  %s%s%s'%(bi,idf,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
